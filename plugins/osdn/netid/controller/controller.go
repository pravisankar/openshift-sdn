...
func (c *Allocation) AddOrUpdate(ns *kapi.Namespace) error {
	...
	// Remove the intent before processing
	...
	if vnidAllocated {
		// Only accept VNIDs that are already allocated
		if !c.vnid.Has(id) {
...
func (c *Allocation) Delete(ns *kapi.Namespace) error {
	...
	for _, n := range nsList.Items {
		...
		glog.Infof("Ignored releasing netid %d for namespace %q (already in use by %q)", id, name, n.ObjectMeta.Name)
		return nil
	}
