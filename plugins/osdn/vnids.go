...
func (oc *OsdnController) VnidStartMaster() error {
	...
	// Run vnid controller
	factory := vnidcontroller.AllocationFactory{
		VNIDAllocator:    netIDAllocator,
		Client:           kclient.Namespaces(),
		GlobalNamespaces: []string{kapi.NamespaceDefault},
	}
...
func (oc *OsdnController) GetVNID(name string) (uint, error) {
	...
	return vnid.MaxVNID + 1, fmt.Errorf("Failed to find netid for namespace: %s in vnid map", name)
}
...
func (oc *OsdnController) watchNamespaces() {
	...
	for {
		...
		case watch.Deleted:
			...
			err := oc.updatePodNetwork(name, vnid.GlobalVNID)
			if err != nil {
				log.Errorf("Failed to update pod network for namespace '%s', error: %v", name, err)
			}
			oc.unSetVNID(name)
