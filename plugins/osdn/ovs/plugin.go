...
func CreatePlugin(registry *osdn.Registry, etcdHelper storage.Interface, multitenant bool, hostname string, selfIP string) (api.OsdnPlugin, api.FilteringEndpointsConfigHandler, error) {
	plugin := &ovsPlugin{multitenant: multitenant}

	err := plugin.BaseInit(registry, plugin, etcdHelper, multitenant, hostname, selfIP)
	if err != nil {
		return nil, nil, err
	}
