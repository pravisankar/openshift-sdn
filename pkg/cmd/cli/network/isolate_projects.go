...
func (i *IsolateOptions) Run() error {
	...
	projNets, err := i.Options.GetProjectNetworks()
	if err != nil {
		return err
	}
	netIDNamespaceMap := make(map[string]uint, len(projNets.Items))
	netIDCountMap := make(map[uint]uint, len(projNets.Items))
	for _, projNet := range projNets.Items {
		netIDNamespaceMap[projNet.ObjectMeta.Name] = *projNet.NetID
		netIDCountMap[*projNet.NetID] += 1
	}
	...
	for _, project := range projects {
		...
		// - Global network namespace (server will only allow this change for cluster admins)
